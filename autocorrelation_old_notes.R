...
##          things closer together in time and/or space tend to be more similar than those further away from each other
...
set.seed(20) ### <-- algorithm that generates random number. this seed will always generate the same set of random numbers
whitenoise <- ts(rnorm(273))
### generate random numbers with rnorm (view ?rnorm for other commands). or rpois
...
### autocorrelation value of 1 : 100% correlated
...
adf.test(whitenoise)
##### data:  whitenoise. Dickey-Fuller = -6.4757, Lag order = 6, p-value = 0.01. alternative hypothesis: stationary
